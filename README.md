...
- 69-case text-only dev evaluation set；
...
当前不建议继续大改 parser。V3.5 结果显示 top-5 evidence recall 已稳定，剩余问题更适合进入 answer generation、UI/API、final benchmark 或延迟优化阶段，而不是继续为个别 top-1 case 增加 parser 特例。
...
它是 69 条 text-only development cases，不是 final benchmark。该数据集用于开发期回归、方案对比和 failure review，不能代表所有车型、品牌或汽车用户手册问题。
...
最新 V3.5 对比来自 `reports/evaluation/v3_5_subsection_whitelist_comparison.md`。
...
| --- | ---: | ---: | ---: | ---: |
| Hybrid no-rerank | 0.8841 | 0.9855 | 1.0000 | 0.9353 |
| Hybrid + rerank | 0.9710 | 1.0000 | 1.0000 | 0.9855 |
...
- rerank latency cost ≈ 2.27x
- no-rerank elapsed ≈ 14.77s
- rerank elapsed ≈ 33.61s

当前结论：

1. V3.5 subsection hierarchy + subsection heading in block text 当前可以接受；
2. `evidence_hit@5 = 1.0000`，说明 top-5 召回稳定；
3. hybrid no-rerank 已经是较好的 baseline；
4. rerank 能显著提升 top-1 evidence ranking；
5. rerank 带来明显延迟成本，应继续 optional；
6. 当前剩余问题不应继续通过 parser 特例硬修；
7. 下一阶段更适合转向总结文档、API/UI、answer generation 质量或 final benchmark 设计。
...
- top-5 evidence recall 已稳定，不应为了少数 top-1 case 无限改 parser。
- rerank 在当前 dev set 上有效，但不默认开启。
- 当前评测主要验证 retrieval，不等同于最终 answer generation 质量。
- 69-case dev set 不是 final benchmark。
...
### 优先级 A：固化当前 checkpoint

- 整理 V3.5 parser / retrieval / rerank summary；
- 将 README 作为当前阶段导航文档；
- 清理未提交文件，区分代码、文档和运行产物；
- 保留关键 reports 的结论，避免提交大体积或临时报告；
- 补充关键命令、开发流程和 commit 边界；
- 为最终对外 README / 简历描述提炼素材。

### 优先级 B：继续提升项目展示和工程完整度

- 设计更严格的 final benchmark；
- 增加 answer generation 质量评估，例如 groundedness、citation correctness、answer coverage；
...
- 当前 eval set 是 69-case dev set，不是 final benchmark。
...
- 构建 69-case text-only dev evaluation set 和 retrieval evaluation runner；
- 通过 evaluation-driven iteration 将 hybrid no-rerank `evidence_hit@1` 提升到 `0.8841`，rerank 后达到 `0.9710`；
