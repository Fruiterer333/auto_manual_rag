...
- generated_at: 2026-09-03T18:12:48
...
- experiment_id: v4.8-final-system-evaluation
- experiment_phase: N/A
- git_branch: exp/v4.8-final-system-evaluation
- git_commit: 65869746f56c676dfa1b9e162fba412c2c69d0eb
...
| must_include_term_average_coverage | 0.8081 |
| expected_evidence_hit_rate | 0.3636 |
| insufficient_evidence_behavior_pass_rate | 0.0000 |
| average_elapsed_seconds | 19.4300 |
...
根据手册内容，关于电子驻车制动（EPB）的启用和释放方法如下：

**1. 启用电子驻车制动（使车辆保持静止）**
您可以使用电子驻车制动（EPB）来使车辆保持静止状态。

**2. 释放电子驻车制动**
*   **常规操作：**
    *   踩下制动踏板并启动车辆；
    *   沿箭头方向按下 EPB 开关，即可释放 EPB 功能，此时红色 EPB 指示灯会自动熄灭。
    *   或者：启动车辆后，将变速器挂入 D 或 R 挡，同时踩下油门踏板也可以释放电子驻车制动。

*   **紧急情况下的操作：**
    *   如果车辆行驶过程中出现紧急情况，您可以**拉住**电子驻车制动器开关，以启用紧急制动功能（注意：此处为“拉住”而非“按下”，用于紧急制动）。
    *   释放该开关后，即可解除紧急制动。

**3. 重要安全警告**
*   **防止溜车：** 在 EPB 处于释放状态时，您必须注意车辆周边环境，以防止发生溜车危险。

**Final answer:**

根据手册内容，关于电子驻车制动（EPB）的启用和释放方法如下： **1. 启用电子驻车制动（使车辆保持静止）**
您可以使用电子驻车制动（EPB）来使车辆保持静止状态。 **2. 释放电子驻车制动**
* **常规操作：** * 踩下制动踏板并启动车辆； * 沿箭头方向按下 EPB 开关，即可释放 EPB 功能，此时红色 EPB 指示灯会自动熄灭。 * 或者：启动车辆后，将变速器挂入 D 或 R 挡，同时踩下油门踏板也可以释放电子驻车制动。 * **紧急情况下的操作：** * 如果车辆行驶过程中出现紧急情况，您可以**拉住**电子驻车制动器开关，以启用紧急制动功能（注意：此处为“拉住”而非“按下”，用于紧急制动）。 * 释放该开关后，即可解除紧急制动。 **3. 重要安全警告**
* **防止溜车：** 在 EPB 处于释放状态时，您必须注意车辆周边环境，以防止发生溜车危险。
...
根据手册内容，使用遥控钥匙启动车辆需遵循以下步骤和条件：

**启动前的必要条件：**
*   **挡位要求**：请将挡位置于驻车挡（P）。
*   **钥匙位置**：将遥控钥匙放置于车内。
*   **安全操作**：将制动踏板踩到底。
*   **钥匙有效性**：只有经过正确编码的遥控钥匙才能启动车辆。
...
**特殊情况（紧急启动）：**
如果遥控钥匙电池电量低或系统无法检测到钥匙时，仪表显示屏会弹出“未检测到钥匙”的消息提示。此时请按以下步骤操作：
1.  将遥控钥匙放入前杯托底部。
2.  按照上述常规步骤启动车辆。

**注意：** 在更换遥控钥匙电池后，请按下任意按键检查工作指示灯是否点亮，并确认遥控钥匙恢复正常工作后再进行启动操作。

**Final answer:**

根据手册内容，使用遥控钥匙启动车辆需遵循以下步骤和条件： **启动前的必要条件：**
* **挡位要求**：请将挡位置于驻车挡（P）。
* **钥匙位置**：将遥控钥匙放置于车内。
* **安全操作**：将制动踏板踩到底。
* **钥匙有效性**：只有经过正确编码的遥控钥匙才能启动车辆。 **具体操作步骤：**
1. 长按 START/STOP 按钮。
2. 车辆启动后，松开按钮。 **特殊情况（紧急启动）：**
如果遥控钥匙电池电量低或系统无法检测到钥匙时，仪表显示屏会弹出“未检测到钥匙”的消息提示。此时请按以下步骤操作：
1. 将遥控钥匙放入前杯托底部。
2. 按照上述常规步骤启动车辆。 **注意：** 在更换遥控钥匙电池后，请按下任意按键检查工作指示灯是否点亮，并确认遥控钥匙恢复正常工作后再进行启动操作。
...
根据车辆用户手册，使用高压水枪清洗车辆时，请务必遵守以下注意事项：

**安全警告与禁止事项：**
*   **严禁冲洗特定部位：** 禁止将水枪对准车辆底部接插件进行冲洗；请勿使喷嘴直接或间接对发动机舱内进行冲洗（高压水流会损坏电气元件或导致部件无法使用）。
*   **避免损伤传感器：** 避免用高压清洗机或蒸汽清洗机对传感器进行清洁。清洗传感器表面时，应使用较小的水流短时间冲洗，且喷嘴与传感器至少保持 10 cm 以上的距离。
*   **控制压力与距离：** 务必严格按高压清洗器使用说明操作。如果使用压力清洗器，喷嘴与车身表面的距离至少须保持 30 cm。
*   **规范操作手法：** 保持喷嘴移动，不要朝某个部位一直喷水，以防高压水流浸入车辆零部件内导致损坏。

**清洗前的准备：**
*   **检查开闭件：** 洗车前，必须检查并确认车辆的外部开闭件已正确关闭。
*   **关闭车窗天窗：** 在清洗车辆外部前，请记得关闭所有车窗和天窗。

**清洗后的处理：**
*   **制动干燥：** 在清洗车辆后若要立即驾车，请数次踩下制动踏板，以去除制动摩擦片上的水汽（防止影响制动效率）。
*   **擦干表面：** 清洗完车辆后，请将车辆表面擦拭干净，否则残留的清洁剂会腐蚀车辆外观。

**清洁剂使用限制：**
*   请勿使用含酸清洁剂、强碱性肥皂、强化学性清洗剂、汽油或溶剂清洗车辆，以免损坏车辆表面或影响外观。

**Final answer:**

根据车辆用户手册，使用高压水枪清洗车辆时，请务必遵守以下注意事项： **安全警告与禁止事项：**
* **严禁冲洗特定部位：** 禁止将水枪对准车辆底部接插件进行冲洗；请勿使喷嘴直接或间接对发动机舱内进行冲洗（高压水流会损坏电气元件或导致部件无法使用）。
* **避免损伤传感器：** 避免用高压清洗机或蒸汽清洗机对传感器进行清洁。清洗传感器表面时，应使用较小的水流短时间冲洗，且喷嘴与传感器至少保持 10 cm 以上的距离。
* **控制压力与距离：** 务必严格按高压清洗器使用说明操作。如果使用压力清洗器，喷嘴与车身表面的距离至少须保持 30 cm。
* **规范操作手法：** 保持喷嘴移动，不要朝某个部位一直喷水，以防高压水流浸入车辆零部件内导致损坏。 **清洗前的准备：**
* **检查开闭件：** 洗车前，必须检查并确认车辆的外部开闭件已正确关闭。
* **关闭车窗天窗：** 在清洗车辆外部前，请记得关闭所有车窗和天窗。 **清洗后的处理：**
* **制动干燥：** 在清洗车辆后若要立即驾车，请数次踩下制动踏板，以去除制动摩擦片上的水汽（防止影响制动效率）。
* **擦干表面：** 清洗完车辆后，请将车辆表面擦拭干净，否则残留的清洁剂会腐蚀车辆外观。 **清洁剂使用限制：**
* 请勿使用含酸清洁剂、强碱性肥皂、强化学性清洗剂、汽油或溶剂清洗车辆，以免损坏车辆表面或影响外观。
...
当制动防抱死系统（ABS）发生故障时，请按照以下步骤处理：

1.  **确认故障指示**
    首先检查仪表显示屏和中央显示屏。如果防抱死制动系统存在故障，相应的警告灯会点亮。同时留意显示屏是否有故障警告指示灯图标或消息提示。

2.  **了解含义与措施**
    如果有警告信息，请查阅“指示灯和警告灯”章节以了解该图标的含义及相应的处理措施。

3.  **检查系统功能状态**
    确认是否有任何系统功能下降或受限。
    *   **注意**：在制动过程中，如果您感觉到机舱传来连续“咯咯”的声音并伴随制动踏板振动，这是防抱死制动系统工作时的正常现象，并非故障。
    *   如果感觉制动踏板恢复到非防抱死制动的状态（即没有上述声音和振动），则表明防抱死制动系统可能已停止工作。

4.  **寻求专业帮助**
    如果您重复检查上述步骤后仍无法解决故障，或者确认系统功能受限且无法自行处理，请立即联系领克中心寻求帮助。

**安全提示：**
当防抱死制动系统停止工作时，车辆将失去在紧急制动情况下的转向控制性能改善能力。请务必谨慎驾驶，避免急刹车导致车轮抱死。

**Final answer:**

当制动防抱死系统（ABS）发生故障时，请按照以下步骤处理： 1. **确认故障指示** 首先检查仪表显示屏和中央显示屏。如果防抱死制动系统存在故障，相应的警告灯会点亮。同时留意显示屏是否有故障警告指示灯图标或消息提示。 2. **了解含义与措施** 如果有警告信息，请查阅“指示灯和警告灯”章节以了解该图标的含义及相应的处理措施。 3. **检查系统功能状态** 确认是否有任何系统功能下降或受限。 * **注意**：在制动过程中，如果您感觉到机舱传来连续“咯咯”的声音并伴随制动踏板振动，这是防抱死制动系统工作时的正常现象，并非故障。 * 如果感觉制动踏板恢复到非防抱死制动的状态（即没有上述声音和振动），则表明防抱死制动系统可能已停止工作。 4. **寻求专业帮助** 如果您重复检查上述步骤后仍无法解决故障，或者确认系统功能受限且无法自行处理，请立即联系领克中心寻求帮助。 **安全提示：**
当防抱死制动系统停止工作时，车辆将失去在紧急制动情况下的转向控制性能改善能力。请务必谨慎驾驶，避免急刹车导致车轮抱死。
...
- answer_non_empty: True
- must_include_term_coverage: 0.0000
- matched_must_include_terms: N/A
- missing_must_include_terms: 安全区域停车; Lynk & Co领克中心
- forbidden_term_hits: N/A
- evidence_structure_hit: False
- expected_section_hit: True
- expected_subsection_hit: False
...
车辆配有制动防抱死系统（ABS），其主要作用如下：

*   **防止车轮抱死**：在您施加最大制动力时，该系统可防止车轮抱死。
*   **改善转向控制**：在大多数路况下，该系统可在紧急制动情况下改善车辆的转向控制性能。

**注意事项：**
*   当防抱死制动系统停止工作时，制动踏板的感觉会恢复到非防抱死制动的状态。
*   如果在崎岖、有碎石或有雪覆盖的道路上行驶，制动距离将比在正常道路时更长。
*   **故障处理**：若发现防抱死制动系统存在故障（警告灯点亮），请立刻在安全区域停车并联系领克中心。

**Final answer:**

车辆配有制动防抱死系统（ABS），其主要作用如下： * **防止车轮抱死**：在您施加最大制动力时，该系统可防止车轮抱死。
* **改善转向控制**：在大多数路况下，该系统可在紧急制动情况下改善车辆的转向控制性能。 **注意事项：**
* 当防抱死制动系统停止工作时，制动踏板的感觉会恢复到非防抱死制动的状态。
* 如果在崎岖、有碎石或有雪覆盖的道路上行驶，制动距离将比在正常道路时更长。
* **故障处理**：若发现防抱死制动系统存在故障（警告灯点亮），请立刻在安全区域停车并联系领克中心。
...
根据手册内容，自动驻车（Auto Hold）的作用如下： * **提供制动支持**：在正常驾驶过程中的短暂停车时，系统会自动提供制动。
* **解放驾驶员脚部**：通过上述功能，您不需要一直踩住制动踏板。
* **起步操作简化**：当车辆需要起步时，您只需踩下加速踏板，或者用力踩下制动踏板再松开，车辆即可正常行驶。
...
根据手册内容，关于超速报警的触发与解除情况如下：

**1. 触发条件**
当车辆行驶速度**高于**您设定的“超速报警车速”时，系统会触发报警。
*   **表现：** 系统会发出声音警告，同时仪表显示屏弹出提示信息提醒驾驶员减慢车速。
*   **注意：** 超速报警功能仅具备提醒作用，无法防止驾驶员超过设定车速，驾驶员必须主动控制车速（E1, E2）。

**2. 解除条件**
当车辆行驶速度**低于**设定的“超速报警车速”时，报警会自动解除。

**3. 特殊情况说明（针对限速偏差）**
如果您设置了“限速偏差数值”，触发和解除的逻辑会包含该偏差值：
*   **触发：** 当车速超过“识别到的限速值 + 限速偏差值”时，系统发出报警。
*   **解除：** 直至车速低于“识别到的限速值 + 限速偏差值”时，报警取消。
*   **防误报机制：** 如果车辆减速至低于上述阈值**5 km/h以下**后再次加速并超过该阈值，系统才会再次触发报警；否则将不再重复触发，直到限速信息更新（E4）。

**Final answer:**

根据手册内容，关于超速报警的触发与解除情况如下： **1. 触发条件**
当车辆行驶速度**高于**您设定的“超速报警车速”时，系统会触发报警。
* **表现：** 系统会发出声音警告，同时仪表显示屏弹出提示信息提醒驾驶员减慢车速。
* **注意：** 超速报警功能仅具备提醒作用，无法防止驾驶员超过设定车速，驾驶员必须主动控制车速（E1, E2）。 **2. 解除条件**
当车辆行驶速度**低于**设定的“超速报警车速”时，报警会自动解除。 **3. 特殊情况说明（针对限速偏差）**
如果您设置了“限速偏差数值”，触发和解除的逻辑会包含该偏差值：
* **触发：** 当车速超过“识别到的限速值 + 限速偏差值”时，系统发出报警。
* **解除：** 直至车速低于“识别到的限速值 + 限速偏差值”时，报警取消。
* **防误报机制：** 如果车辆减速至低于上述阈值**5 km/h以下**后再次加速并超过该阈值，系统才会再次触发报警；否则将不再重复触发，直到限速信息更新（E4）。
...
根据手册内容，出现以下任一情况时，**无法通过网络远程启动发动机**：
...
此外，若手机在车外且 Lynk & Co APP 处于关闭状态，也无法通过蓝牙方式控制车辆启动。

**Final answer:**

根据手册内容，出现以下任一情况时，**无法通过网络远程启动发动机**： * **挡位不在驻车挡（P）**；
* **冷却液液位较低**；
* **燃油油位较低**；
* **车内有遥控钥匙**；
* **网络信号丢失**；
...
* **发动机出现故障**（例如出现发动机关键故障代码）；
* **车辆设置为维修模式**；
* **连续两次远程启动车辆后，未手动启动车辆**。 此外，若手机在车外且 Lynk & Co APP 处于关闭状态，也无法通过蓝牙方式控制车辆启动。
...
当 EPB AUTO 功能开启后，如果您在车辆处于静止状态时将车辆熄火，车辆将自动启用电子驻车制动（EPB）。
...
在洗车或牵引车辆等特殊情况下，若需要在车辆熄火后让电子驻车制动（EPB）保持释放状态，请按照以下任一方式操作：

**方式一：**
1. 确保车辆处于启动状态。
2. 踩住制动踏板，将变速器挡位切换至 N 挡。
3. 在中央显示屏中进入“车辆设置 - 其他”，关闭"EPB AUTO"功能。
4. 按下 EPB 开关，将 EPB 切换至释放状态。
5. 长按 START/STOP 按钮使车辆熄火。

**方式二：**
1. 确保车辆处于熄火状态（不踩制动踏板）。
2. 长按 START/STOP 按钮 7 秒，待车辆完成自检后。
3. 踩住制动踏板，将变速器切换至 N 挡。
4. 按下 EPB 开关，将 EPB 切换至释放状态。
5. 再次长按 START/STOP 按钮使车辆下电。

**⚠️ 重要安全警告：**
在 EPB 处于释放状态时，您必须时刻注意车辆周边环境，防止车辆溜车造成危险。

**Final answer:**

在洗车或牵引车辆等特殊情况下，若需要在车辆熄火后让电子驻车制动（EPB）保持释放状态，请按照以下任一方式操作： **方式一：**
1. 确保车辆处于启动状态。
2. 踩住制动踏板，将变速器挡位切换至 N 挡。
3. 在中央显示屏中进入“车辆设置 - 其他”，关闭"EPB AUTO"功能。
4. 按下 EPB 开关，将 EPB 切换至释放状态。
5. 长按 START/STOP 按钮使车辆熄火。 **方式二：**
1. 确保车辆处于熄火状态（不踩制动踏板）。
2. 长按 START/STOP 按钮 7 秒，待车辆完成自检后。
3. 踩住制动踏板，将变速器切换至 N 挡。
4. 按下 EPB 开关，将 EPB 切换至释放状态。
5. 再次长按 START/STOP 按钮使车辆下电。 **⚠️ 重要安全警告：**
在 EPB 处于释放状态时，您必须时刻注意车辆周边环境，防止车辆溜车造成危险。
...
根据手册内容，车辆充电输入插座上的 LED 指示灯会根据不同的充电状态显示不同颜色，具体含义如下：
...
*   **红色常亮**：表示充电故障。
    *   **注意**：若出现此情况，请立即拔出充电枪并锁止车辆，等待车辆休眠后再尝试充电。如果红色指示灯持续点亮，请联系领克中心寻求帮助。
*   **橙色常亮**：表示充电预加热中。
*   **橙 - 橙 - 红依次闪烁**：表示充电预加热中电子锁故障。

此外，在仪表显示屏上会显示估计的剩余充电时间，此时充电接口插座中的 LED 指示灯也会闪绿光。

**Final answer:**

根据手册内容，车辆充电输入插座上的 LED 指示灯会根据不同的充电状态显示不同颜色，具体含义如下： * **蓝色常亮（2 分钟后熄灭）**：表示预约充电。
* **蓝色闪烁**：表示对外放电。
* **绿色闪烁**：表示正在充电。
* **绿色常亮**：表示充电完成。
* **绿 - 绿 - 红依次闪烁**：表示充电中电子锁故障。
* **红色常亮**：表示充电故障。 * **注意**：若出现此情况，请立即拔出充电枪并锁止车辆，等待车辆休眠后再尝试充电。如果红色指示灯持续点亮，请联系领克中心寻求帮助。
* **橙色常亮**：表示充电预加热中。
* **橙 - 橙 - 红依次闪烁**：表示充电预加热中电子锁故障。 此外，在仪表显示屏上会显示估计的剩余充电时间，此时充电接口插座中的 LED 指示灯也会闪绿光。
...
根据提供的用户手册内容，**没有说明**如何通过语音命令校准制动系统。

依据如下：
1.  **关于语音助手的控制范围**：手册指出您可以通过语音助手对车辆功能（例如：车窗、天窗、空调等）进行语音控制，但未提及包含“制动系统校准”或相关操作指令。
2.  **关于制动系统的说明**：手册在“制动防抱死系统”部分仅说明了制动过程中出现声音和振动是正常现象，以及防抱死系统停止工作时的踏板感觉变化，并未涉及任何通过语音进行校准的步骤或方法。
3.  **关于制动故障的处理**：若仪表显示制动系统故障指示灯，手册要求立刻停车并联系服务中心，未提及可通过语音命令解决或校准。

因此，基于现有证据，无法找到通过语音命令校准制动系统的依据。

**Final answer:**

根据提供的用户手册内容，**没有说明**如何通过语音命令校准制动系统。 依据如下：
1. **关于语音助手的控制范围**：手册指出您可以通过语音助手对车辆功能（例如：车窗、天窗、空调等）进行语音控制，但未提及包含“制动系统校准”或相关操作指令。
2. **关于制动系统的说明**：手册在“制动防抱死系统”部分仅说明了制动过程中出现声音和振动是正常现象，以及防抱死系统停止工作时的踏板感觉变化，并未涉及任何通过语音进行校准的步骤或方法。
3. **关于制动故障的处理**：若仪表显示制动系统故障指示灯，手册要求立刻停车并联系服务中心，未提及可通过语音命令解决或校准。 因此，基于现有证据，无法找到通过语音命令校准制动系统的依据。
...
- expected_subsection_hit: N/A
- insufficient_evidence_behavior: False
- internal_reference_leak: False
